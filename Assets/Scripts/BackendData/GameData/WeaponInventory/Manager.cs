...
namespace BackendData.GameData.WeaponInventory
{
    ...
    public class Manager : Base.GameData
    {
        ...
        public string AddWeapon(int weaponId)
        {
            ...
            if (StaticManager.Backend.GameData.WeaponInventory.Dictionary.ContainsKey(weaponId.ToString()))
            {
                dup = true;
            }
            else
            {
                _dictionary.Add(weaponId.ToString(), new Item(weaponId.ToString(), 1, weaponInfo));
                IsChangedData = true;
            }
