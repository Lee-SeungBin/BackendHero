                ...
                switch (chartType)
                {
                    case ChartType.forPost:
                        ...
                        if (itemID == 1)
                        {
                            ...
                            _receiveFunc = () => { Managers.Game.UpdateUserData(itemCount, 0, 0); };
                        }
                        else if (itemID == 2)
                        {
                            itemName = "jewel";
                            _receiveFunc = () => { Managers.Game.UpdateUserData(0, 0, itemCount); };
                        }
                        else if (itemID == 3)
                        {
                            itemName = "exp";
                            //Receive 함수 호출 시 해당 델리게이트 호출
                            _receiveFunc = () => { Managers.Game.UpdateUserData(0, itemCount, 0); };
                        }
                        break;
